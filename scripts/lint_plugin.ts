...
// Adapted from Deno std library lint plugin.
...
export default {
  ...
  rules: {
    ...
    "naming-convention": {
      create(context) {
        // Preact components use PascalCase function names — skip .tsx files
        ...
        return {
          ...
          FunctionDeclaration(node) {
            ...
            const name = id.name;
            if (!name) return;
            // Allow PascalCase in component files (Preact components)
            if (isComponentFile && isPascalCase(name)) return;
            if (!isCamelCase(name)) {
              context.report({
                node: id,
    ...
    "exported-function-args-maximum": {
      create(context) {
        ...
        // test files don't need to follow this rule
        if (url.href.includes("_test.")) return {};
            ...
            if (params.length === 3) {
              ...
              switch (param.type) {
                case "Identifier": {
                  ...
                  // Function as 3rd argument is valid
                  if (
