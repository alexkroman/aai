...
Deno.test("resample", async (t) => {
  ...
  await t.step("downsamples 24kHz to 16kHz (3:2 ratio)", () => {
    ...
    const out = resample(input, { fromRate: 24000, toRate: 16000 });
  ...
  await t.step("upsamples 16kHz to 24kHz (2:3 ratio)", () => {
    ...
    const out = resample(input, { fromRate: 16000, toRate: 24000 });
  ...
  await t.step("single sample input produces output", () => {
    const out = resample(new Float32Array([0.7]), {
      fromRate: 48000,
      toRate: 16000,
    });
    assert(out.length >= 1);
