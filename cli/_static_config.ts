 ...
 */
export async function extractStaticConfig(
  agentPath: string,
...
export function extractStaticConfigFromSource(
  ...
  if (!arg || !arg.isKind(SyntaxKind.ObjectLiteralExpression)) {
    throw new BundleError(
      "The argument to defineAgent() must be an inline object literal",
    );
  }
...
function findDefineAgentCall(sf: SourceFile): CallExpression | undefined {
  return sf.getFirstDescendant((node) => {
    ...
    return expr.isKind(SyntaxKind.Identifier) &&
      expr.getText() === "defineAgent";
...
function extractConfig(
  ...
  if (transportProp) {
    ...
    if (init.isKind(SyntaxKind.StringLiteral)) {
      ...
      config.transport = evalStringArray(
        init,
        "transport",
        fileName,
      ) as AgentConfig["transport"];
    } else {
      throw new BundleError(
...
function extractToolSchemas(
  ...
  if (!init.isKind(SyntaxKind.ObjectLiteralExpression)) {
    throw new BundleError(
      `${fileName}: \`tools\` must be an inline object literal.`,
    );
  }
  ...
  for (const member of toolsObj.getProperties()) {
    ...
    if (!toolInit.isKind(SyntaxKind.ObjectLiteralExpression)) {
      throw new BundleError(
        `${fileName}: Tool \`${toolName}\` must be an inline object literal.`,
      );
    }
    ...
    if (!descProp) {
      throw new BundleError(
        `${fileName}: Tool \`${toolName}\` is missing a \`description\`.`,
      );
    }
    const description = evalStringLiteral(
      descProp.getInitializer()!,
      toolName,
      fileName,
    );
...
function parseZodExpr(
  ...
  if (expr.isKind(SyntaxKind.PropertyAccessExpression)) {
    ...
    if (method === "describe") {
      ...
      const args = call.getArguments();
      if (args[0]) {
        result.schema.description = evalStringLiteral(
...
function requireString(
  ...
  if (!prop) {
    throw new BundleError(
      `${fileName}: The \`${field}\` field is required in defineAgent({...}).`,
    );
  }
...
function evalStringLiteral(
  ...
  if (node.isKind(SyntaxKind.NoSubstitutionTemplateLiteral)) {
    return node.asKindOrThrow(SyntaxKind.NoSubstitutionTemplateLiteral)
      .getLiteralValue();
  }
...
function evalStringArray(
  ...
  if (!node.isKind(SyntaxKind.ArrayLiteralExpression)) {
    throw new BundleError(
      `${fileName}: \`${context}\` must be an array literal.`,
    );
  }
  const arr = node.asKindOrThrow(SyntaxKind.ArrayLiteralExpression);
  return arr.getElements().map((el) =>
    evalStringLiteral(el as Expression, context, fileName)
  );
}
...
function evalLiteral(
  ...
  if (node.isKind(SyntaxKind.NumericLiteral)) {
    return Number(
      node.asKindOrThrow(SyntaxKind.NumericLiteral).getLiteralValue(),
    );
  }
  ...
  if (node.isKind(SyntaxKind.ArrayLiteralExpression)) {
    return node.asKindOrThrow(SyntaxKind.ArrayLiteralExpression)
      .getElements()
