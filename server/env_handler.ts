...
export async function handleEnvList(
  ...
  if (!env) throw new HttpError(404, `Agent ${slug} not found`);
...
export async function handleEnvSet(
  ...
  if (
    ...
    throw new HttpError(400, "Body must be a JSON object of key-value pairs");
  }

  for (const [k, v] of Object.entries(updates)) {
    if (typeof v !== "string") {
      throw new HttpError(400, `Value for "${k}" must be a string`);
    }
  }
...
export async function handleEnvDelete(
  ...
  const { slug, key } = opts;
  const existing = await ctx.state.store.getEnv(slug);
  if (!existing) throw new HttpError(404, `Agent ${slug} not found`);
